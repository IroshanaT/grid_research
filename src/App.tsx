          ...
          <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
            <div>
              <h1 className="text-2xl font-bold tracking-tight">
                Grid 12G/8G Layout Tool
              </h1>
              <p className="text-sm text-gray-500">
                Visualize blocks positioned on the 12-column or 8-column grid in
                a single row.
              </p>
            </div>
            <div className="flex items-center gap-4">
              ...
              <select
                ...
                value={granularity}
                onChange={(e) =>
                  setGranularity(parseFloat(e.target.value) as any)
                }
              >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                <label className="flex flex-col gap-1 text-sm">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateBlock({ gridSystem: e.target.value as GridSystem })
                    }
                  >
                ...
                <label className="flex flex-col gap-1 text-sm">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateBlock({ type: e.target.value as BlockType })
                    }
                  >
                ...
                <label className="flex flex-col gap-1 text-sm">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateBlock({ startCol: parseFloat(e.target.value) || 1 })
                    }
                  />
                </label>
                ...
                <label className="flex flex-col gap-1 text-sm">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateBlock({ startRow: parseInt(e.target.value) || 1 })
                    }
                  />
                </label>
                ...
                <label className="flex flex-col gap-1 text-sm">
                  ...
                  <input
                    ...
                    step={granularity}
                    max={
                      activeBlock.gridSystem === "12G"
                        ? 12 - activeBlock.startCol + 1
                        : 8 - activeBlock.startCol + 1
                    }
                    className="border border-gray-300 rounded px-2 py-1.5 outline-none focus:border-blue-500"
                    value={activeBlock.span}
                    onChange={(e) =>
                      updateBlock({
                        span: parseFloat(e.target.value) || granularity,
                      })
                    }
                  />
                </label>
      ...
      <div className="flex-1 w-full bg-[#f0f2f5] py-10 flex flex-col items-center">
            ...
            <div className="relative w-full h-full pt-20 pointer-events-auto flex flex-col gap-6">
              {Array.from({ length: rowCount }).map((_, rIndex) => {
                ...
                return (
                  <div
                    key={currentRow}
                    className="grid-renderer relative w-full h-[64px] bg-gray-100/50 border-t border-b border-dashed border-gray-400 rounded-sm"
                  >
                    <div className="absolute -top-5 left-0 text-[10px] font-mono text-gray-500 font-bold bg-white px-1">
                      Row {currentRow}
                    </div>

                    {blocks
                      .filter((b) => b.startRow === currentRow)
                      .map((block) => {
                        ...
                        return (
                          ...
                          >
                            {block.type === "label" ? (
                              <div
                                className={`cursor-pointer flex items-center justify-center h-full bg-[#e8eaed] text-gray-700 px-3 border border-gray-400 rounded-sm text-[13px] font-medium overflow-hidden ${activeBlockId === block.id ? "ring-2 ring-blue-500 ring-offset-1 z-10 relative" : ""}`}
                              >
                              ...
                              <div
                                className={`cursor-pointer flex flex-col justify-center h-full bg-[#f8f9fa] px-1 border border-gray-400 rounded-sm shadow-inner overflow-hidden ${activeBlockId === block.id ? "ring-2 ring-blue-500 ring-offset-1 z-10 relative" : ""}`}
                              >
                                <input
                                  type="text"
                                  disabled
        ...
        <div className="relative w-full text-sm text-gray-600 mt-8 max-w-3xl bg-white p-6 border border-gray-200 rounded shadow-sm">
          ...
          <ul className="list-disc pl-5 space-y-2">
            <li>
              <strong className="text-red-500">Red columns:</strong> 12G Grid
              system
            </li>
            <li>
            ...
            <li>
              Changing the container width accurately scales both systems
              together, maintaining exactly the calculations defined:{" "}
              <code className="bg-gray-100 px-1 py-0.5 rounded text-pink-600">
                left margin + ((no. after N - 1) * (N size per column + column
                gapsize))
              </code>
              .
            </li>
            <li>
              The blocks are overlaid in the{" "}
              <strong>same horizontal row container</strong> using CSS{" "}
              <code className="bg-gray-100 px-1 py-0.5 rounded text-pink-600">
                calc()
              </code>{" "}
              to position perfectly on either scale!
            </li>
          </ul>
        </div>
      </div>
